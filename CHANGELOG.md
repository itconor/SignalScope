...
Three improvements to the CueServer LED workflow:

**1. Brightness is now per-brand (not per-strip)**
The brightness slider has moved from each studio DMX strip row to the brand/station settings. Every brand can have a different brightness (0–100%) for the same physical strip wiring. Existing per-strip brightness values are ignored going forward; set each brand's brightness in the CueServer LED section of its brand form. DMX strip rows now show channel numbers only (R/G/B/W) — no brightness slider.

**2. Zone Name on studio for `ZONE "Name" Preset N ON` format**
Studios now have an optional **Zone Name** field (e.g. `Studio1LED`). When set, the command auto-generated by "Save as Cue" and the scene list "Use" button uses `ZONE "Studio1LED" Preset N ON` instead of `Cue N Go`. This matches the command format used in the previous Node-RED / UDP workflow. Leave blank to keep the `Cue N Go` format.

**3. Auto-save on scene selection / cue recording**
After clicking "Save as Cue" (records DMX + saves cue number) or clicking "Use" on a scene in the list, the brand is saved automatically — no separate Save click required. The status line confirms with `✓ Command saved: …`. The command field placeholder also now shows both formats as examples.
